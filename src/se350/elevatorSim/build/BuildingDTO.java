...
/**
 * A Data holder class that contains all the specific statistics about each
 * simulation.
 * 
...
public class BuildingDTO {
	...
	/**
	 * The factor by which the simulated time will be sped up in relation to
	 * real time.
	 */
	 ...
	 * 
	 * @param simulationTimeIn
	 *            - Simulation time
	 * @param timeScaleFactorIn
	 *            - Number of simulated seconds per real second
	 * @param numOfFloorsIn
	 *            - Number of floors in the building
	 * @param numOfElevatorsIn
	 *            - number of elevators in the building
	 * @param maxPersonsPerElevatorIn
	 *            - Max capacity of the elevators
	 * @param msPerFloorIn
	 *            - Time for elevators to travel a floor
	 * @param msDoorOperationIn
	 *            - Time for elevators to exchange at a floor
	 * @param defaultFloorIn
	 *            - Default floor for the elevators
	 * @param personsPerMinuteIn
	 *            - Number of people generated per minute
	 * @param floorStatsPercentagesIn
	 *            - Statics of the floors of the building
	 * @throws InvalidParameterException
	 */
	public BuildingDTO(int simulationTimeIn, int timeScaleFactorIn,
			int numOfFloorsIn, int numOfElevatorsIn,
			int maxPersonsPerElevatorIn, int msPerFloorIn,
			int msDoorOperationIn, int defaultFloorIn[],
			int personsPerMinuteIn, int[] floorStatsPercentagesIn)
			throws InvalidParameterException {
	 ...
	 *            - The number of simulation seconds for every regular second.
	 */
	private void setTimeScaleFactor(int timeScaleFactor)
			throws InvalidParameterException {
		if (timeScaleFactor <= 0)
			throw new InvalidParameterException(
					"Simulation time scale factor must be greater than 0");
	...
	private void setMaxPersonsPerElevator(int maxPersonsPerElevator)
		...
		if (maxPersonsPerElevator <= 0)
			throw new InvalidParameterException(
					"Elevator max capacity must be greater than 0");
	 ...
	 */
	private void setMsPerFloor(int msPerFloor) throws InvalidParameterException {
		if (msPerFloor <= 0)
			throw new InvalidParameterException(
					"Milliseconds Per Floor must be greater than 0");
	 ...
	 *            - The time it takes to open, add/remove people and close
	 */
	private void setMsDoorOperation(int msDoorOperation)
			throws InvalidParameterException {
		if (msDoorOperation <= 0)
			throw new InvalidParameterException(
					"Door operation time must be greater than 0");
	 ...
	 * @param elevator
	 *            - The number of the elevator to get the default floor for
	 */
	 ...
	 * 
	 * @param defaultFloor
	 *            - The floor Elevators should default to
	 * @throws InvalidParameterException
	 */
	private void setDefaultFloor(int defaultFloor[])
		...
		if (defaultFloor.length != getNumOfElevators())
			throw new InvalidParameterException(
					"Not the right number of default floors in input file.");

		for (int i = 0; i < defaultFloor.length; i++) {
			if (defaultFloor[i] <= 0 || defaultFloor[i] > getNumOfFloors())
				throw new InvalidParameterException(
						"Default floor for Elevator" + i + 1
								+ " in input file is not a valid floor");
		}
	...
	/**
	 * Sets the Persons generated per minute in the simulation, used in
	 * initialization.
	 * 
	 * @param personsPerMinute
	 *            - Persons generated per minute
	 * @throws InvalidParameterException
	 */
	 ...
	 * 
	 * @param floorStatsPercentages
	 *            - Array of percentages per floor to generate Person instances.
	 * @throws InvalidParameterException
	 */
	private void setFloorStatsPercentages(int[] floorStatsPercentages)
		...
		if (floorStatsPercentages.length != getNumOfFloors())
			throw new InvalidParameterException(
					"Your floor stats do not equal the number of floors");
