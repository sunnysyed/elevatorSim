...
/**
 * An implementation of the Person interface that creates a simple person who
 * starts on one floor and desires to go to another floor through an elevator
 * instance.
 * 
...
public class PersonImpl implements Person {
	...
	/**
	 * The current state of the Person as defined by the enum in the Person
	 * interface.
	 * 
	 * @see #getStatus()
	 */
	...
	/**
	 * The second of the simulation that the person was created, useful for
	 * creating statistics.
	 */
	private int createdTime;

	/**
	 * The second of the simulation that the person moved from the floor to an
	 * elevator.
	 */
	private int addedToElevatorTime;

	/**
	 * The second of the simulation that the person arrived at their
	 * destination.
	 */
	 ...
	 * 
	 * @param id
	 *            - The unique id for a Simple Person
	 * @param dest
	 *            - The floor that the Person wants to get to
	 * @throws InvalidParameterException
	 *             if the destination is outside the number of floors.
	 */
	public PersonImpl(int id, int source, int dest)
			throws InvalidParameterException {
		setId(id);
		setSource(source);
		setDestination(dest);
	 ...
	 * 
	 * @param dest
	 *            - The floor the Person should get to.
	 * @throws InvalidParameterException
	 *             - if the destination is outside the number of floors.
	 */
	private void setDestination(int dest) throws InvalidParameterException {
		if (dest < 0 || dest > FloorManager.getInstance().getNumberOfFloors())
			throw new InvalidParameterException("Person's destination " + dest
					+ " outside of building constraints.");

		this.destination = dest;
	}

	/**
	 * Get the destination of the Person object, defined by the Person
	 * interface.
	 * 
	...
	private void setSource(int source) throws InvalidParameterException {
		if (source < 0
				|| source > FloorManager.getInstance().getNumberOfFloors())
			throw new InvalidParameterException("Person's source " + source
					+ " outside of building constraints.");
	 ...
	 * 
	 * @param idIn
	 *            - the id you want to give to the person
	 * @throws InvalidParameterException
	 *             if the new id is not a positive number.
	 */
	private void setId(int idIn) throws InvalidParameterException {
		if (idIn <= 0)
			throw new InvalidParameterException("Person id must be positive");

		id = idIn;
	}

	/**
	 * Get the status of the Person, as defined by the enum in the Person
	 * interface.
	 * 
	...
	/**
	 * Set the status of the Person, as defined by the enum in the Person
	 * interface.
	 * 
	 * @param newStatus
	...
	/**
	 * Called when the Person is added to the Elevator to measure their wait
	 * time.
	 */
	...
	/**
	 * Called when the Person arrives at their destination to measure their ride
	 * time. Also reports the data to the Report Generator.
	 */
	public void setFinishedTime() {
		...
		PersonResults result = new PersonResults();
		result.id = getId();
	...
	/**
	 * Defines when two SimplePerson objects are equals, useful in removing from
	 * Collections.
	 * 
