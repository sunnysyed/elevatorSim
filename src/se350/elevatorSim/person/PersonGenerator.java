...
/**
 * Singleton that is responsible for generating random people and giving them
 * destinations based on the input data, but also has a static method to work as
 * a Factory. Creating another factory class would be too bulky, so it was
...
public class PersonGenerator {
	...
	/**
	 * Keeps track of the number of people generated so each person has a unique
	 * id
	 */
	private static int currentId = 0;

	/**
	 * Statistic of how many people to generate per simulated minute. Gathered
	 * from input text file.
	 */
	private static int personsPerMinute;

	/**
	 * The number of seconds in between each generated person, determined by 60
	 * / personsPerMinute
	 */
	private static int secondsBetween;

	/**
	 * Used to generate People every 'secondsBetween' seconds. When it reaches
	 * zero, it generates a new Person, then gets reset to secondsBetween.
	 */
	private static int timer;

	/**
	 * Percentages that somebody will be generated at each floor or their
	 * destination will be that floor. Each story refers to index-1
	 */
	private static int[] floorStats;

	/**
	 * A random number generator useful for putting Persons on random floors and
	 * giving them random destinations.
	 */
	...
	public static PersonGenerator getInstance() {
		if (generator == null)
			throw new IllegalStateException(
					"Person Generator must be initialized first.");

		return generator;
	}

	/**
	 * Initializes the PersonGenerator with data that affects how and when
	 * people will be created
	 * 
	 * @param buildingStats
	 *            - Holds all of the data necessary for creating the
	 *            PersonGenerator
	 */
	public static void initialize(BuildingDTO buildingStats) {
		...
		if (counterTo100 != 100)
			throw new IllegalStateException(
					"Floor statistics do not add up to 100");
	}

	/**
	 * Method to create people and add them to floors. Called every second, but
	 * only creates people based on the number of peoplePerMinute specified by
	 * the text file input. They are created every 'secondsBetween' seconds.
	 * 
	 * @return 0 if nothing was generated or the new person's destination if
	 *         somebody was
	 * @throws InvalidParameterException
	 *             if Person creation data is invalid
	 */
	public int generateAndAddPerson() throws InvalidParameterException {
		...
		if (timer == 0) {
			...
			System.out.println(String.format(
					"%s Person %d added at %d pressed %s to go to %d", Building
							.getTimeString(), currentId, source,
					(dest > source ? "UP" : "DOWN"), dest));

			FloorManager.getInstance().pressControlBoxAt(
					source,
					(dest > source ? ElevatorController.UP
	 ...
	 * 
	 * @param sourceFloor
	 *            - The floor the person begins on, cannot return this floor.
	...
	/**
	 * Creates Person implementations that best fit the parameters in a Factory
	 * Pattern type of way.
	 * 
	 * @param id
	 *            - Unique id of the Person instance to be generated
	 * @param source
	 *            - The starting floor of the new Person implementation
	 * @param dest
	 *            - The destination of the new Person implementation
	 * @return Best fit implementation of an Person
	 * @throws InvalidParameterException
	 *             if the destination is not a valid floor
	 */
	public static Person build(int id, int source, int dest)
			throws InvalidParameterException {
		return new PersonImpl(id, source, dest);
	}
